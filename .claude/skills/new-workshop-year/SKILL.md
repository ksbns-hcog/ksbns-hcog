...
6. Do not modify any other year's folder.
7. Remind the user to review content accuracy against the source PDFs before committing, then commit/push per the repo's normal workflow (`git add -A && git commit -m "..." && git push`) — only if the user asks you to commit.
